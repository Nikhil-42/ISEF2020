...
def to_categorical(narry, classes: int) -> list:
    return np.array([[0 if val!=i else 1 for i in range(classes)] for val in narry])

def from_categorical(narray) -> list:
    return np.array([array.index(max(array)) for array in narray])
...
print(from_categorical(network.predict(test_images[:5])), "===", from_categorical(test_labels[:5]))